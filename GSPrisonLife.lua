...
World:Toggle("Respawn at grave", "RG", false, function(val)
    if val then
		local function death()
            ...
            if character then
                ...
                local respawnPosition = humanoidRootPart.Position

                local characterAddedConnection
                characterAddedConnection = player.CharacterAdded:Connect(function(newCharacter)
                    newCharacter:WaitForChild("HumanoidRootPart")

                    -- Delay a short time to ensure that the new character has loaded properly
                    wait(0.1)
                    for i=1,100 do
                        newCharacter:SetPrimaryPartCFrame(CFrame.new(respawnPosition))
                        wait(0.01)
                    end
                    print("done")
                    characterAddedConnection:Disconnect()
                end)
				Respawn = game.Players.LocalPlayer.Character.Humanoid.Died:Once(death)
